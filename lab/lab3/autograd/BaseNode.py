...
class Attention(Node):
    ...
    def cal(self, X):
        ...
        #print(k.transpose(0, 2, 1).shape)
        score = q @ k.transpose(0, 2, 1) / np.sqrt(X.shape[2]) #后两维是sequence_len和feature_dim，score是(B, L, L)形状的
        #print(score.shape)
        exp_score = np.exp(score - np.max(score, axis=-1 ,keepdims=True))
        softmax_score = exp_score / np.sum(exp_score, axis=-1, keepdims=True)
...
class ResLinear(Node):
    ...
    def cal(self, X):
        ...
        # 即Linear(X)+X
        self.cache.append(X)
        return X.dot(self.params[0]) + self.params[1] + X
        raise NotImplementedError
    ...
    def backcal(self, grad):
        ...
        self.grad.append(np.dot(X_reshaped.T, grad_reshaped)) #关于params[0]的导数
        self.grad.append(grad_reshaped.sum(axis=0)) #关于params[1]的导数
        return (grad_reshaped.dot(self.params[0].T)).reshape(*X.shape) + grad #关于X的导数
        raise NotImplementedError
